...
class FloatLabel(QWidget):
    ...
    def __init__(self, cfg: dict):
        ...
        self.news_alert_config  = NewsSource.normalize_news_alert_config(cfg.get("news_alert_config", {}))
        ...
        self._news_items = NewsCache.load_news_cache()
        if self._news_items:
            cached_ids = [str(item.get("id") or "") for item in self._news_items]
            self._news_seen_ids = self._normalize_news_seen_ids(cached_ids + self._news_seen_ids)
            self._news_alert_initialized = True
            if not self._news_last_source:
                self._news_last_source = str(self._news_items[0].get("source") or "").strip()
    ...
    def current_config(self):
        return {
            ...
            "news_alert_config": NewsSource.normalize_news_alert_config(self.news_alert_config),
            "news_alert_initialized": bool(getattr(self, "_news_alert_initialized", False)),
            "news_seen_ids": self._normalize_news_seen_ids(getattr(self, "_news_seen_ids", [])),
            "news_last_source": str(getattr(self, "_news_last_source", "") or ""),
        }

    @staticmethod
    def _normalize_news_seen_ids(values):
    ...
    def _compact_desktop_alert_text(text):
        ...
        picked = [lines[0]]
        for line in lines[1:]:
            if line.startswith(("标的：", "代码：", "盈亏：", "成本价：", "止损线：", "止盈线：", "变动：", "状态：", "来源：", "时间：")):
                picked.append(line)
            if len(picked) >= 6:
                break
        return "\n".join(picked)

    def show_desktop_alert(self, text, ignore_key=None, force=False):
        notifications = normalize_strategy_alert_config(getattr(self, "strategy_alert_config", {}))["notifications"]
        if not force and not notifications.get("desktop_popup"):
            return
    ...
    def _apply_news_items(self, items, seed_only=False, manual=False):
        ...
        if not clean_items:
            return False
        previous = self._normalize_news_seen_ids(getattr(self, "_news_seen_ids", []))
        previous_set = set(previous)
        fetched_ids = [str(item.get("id")).strip() for item in clean_items]
        self._news_seen_ids = self._normalize_news_seen_ids(fetched_ids + previous)
        ...
        self._news_items = NewsCache.merge_news_items(cached, clean_items)
        if self._news_items != cached:
            self._schedule_news_cache_save()
        ...
        if unseen and config.get("enabled"):
    ...
    def _schedule_news_state_save(self):
        if getattr(self, "_news_save_pending", False):
            return
        self._news_save_pending = True
        QTimer.singleShot(60000, self._flush_news_state_save)

    def _flush_news_state_save(self):
        if not getattr(self, "_news_save_pending", False):
            return
        self._news_save_pending = False
        self._notify_change()

    def _schedule_news_cache_save(self):
        if getattr(self, "_news_cache_save_pending", False):
            return
        self._news_cache_save_pending = True
        QTimer.singleShot(3000, self._flush_news_cache_save)

    def _flush_news_cache_save(self):
        if not getattr(self, "_news_cache_save_pending", False):
            return
        self._news_cache_save_pending = False
        try:
            NewsCache.save_news_cache(getattr(self, "_news_items", []))
        except Exception:
            pass
    ...
    def _poll_news(self, force=False):
        ...
        if bootstrap:
            self._news_future = executor.submit(
                NewsSource.fetch_sina_recent_news,
                self._news_http,
                3,
                50,
            )
    ...
    def _poll_news_future(self):
        ...
        if future is None:
            return
        if not future.done():
            QTimer.singleShot(50, self._poll_news_future)
            return
        self._news_future = None
        seed_only = bool(getattr(self, "_news_future_seed_only", False))
        self._news_future_seed_only = False
        manual = bool(getattr(self, "_news_future_manual", False))
        self._news_future_manual = False
        try:
            self._apply_news_items(future.result(), seed_only=seed_only, manual=manual)
        except Exception:
            pass
    ...
    def _show_news_panel(self, items=None, auto_show=False):
        if items:
            self._news_items = NewsCache.merge_news_items(getattr(self, "_news_items", []), items)
    ...
    def set_news_alert_config(self, config):
        old_config = NewsSource.normalize_news_alert_config(getattr(self, "news_alert_config", {}))
        self.news_alert_config = NewsSource.normalize_news_alert_config(config)
    ...
    def shutdown_background(self):
        ...
        try:
            ...
            pass
        try:
            news_executor = getattr(self, "_news_executor", None)
            ...
            pass
        try:
            news_http = getattr(self, "_news_http", None)
            if news_http is not None:
                news_http.close()
        except Exception:
            pass
